// Downloads files from internet archive
const child_process = require('child_process');
const vdf_parser = require('vdf-parser');
const fs = require("fs");
const { createHash } = require('crypto');
const manifestsToDownload = [{name: "steam_client_ubuntu12", timestamp: 20230930004105, rootkey: "ubuntu12"}, {name: "steam_client_win32", timestamp: 20230930002005, rootkey: "win32"}];
...
manifestsToDownload.forEach(mfdata => {
  const timestamp = mfdata.timestamp;
  const baseurl = `http://web.archive.org/web/${timestamp}if_/media.steampowered.com/client/`;
  ...
  fs.writeFileSync("./data/client/"+mfdata.name, manifest);
